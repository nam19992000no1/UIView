...
@implementation DemoColor{
 ...
 - (void)viewDidLoad {
    ...
    colorPatterIndex = 0;
 [self performSelector:@selector(animatedColor:)
               withObject:[[NSNumber alloc]initWithInt:colorPatterIndex]
               afterDelay:1];
    
}

-(void) animatedColor:(NSNumber*) cpIndex {
    ...
    [UIView animateWithDuration:3.0 animations:^{
        ...
        [UIView animateWithDuration:1.0
                         ...
                         completion:^(BOOL finished) {
                             self->colorPatterIndex++;
                             if (self->colorPatterIndex == self->cpArray.count) {
                                 self->colorPatterIndex = 0;
                             }
                             [self animatedColor:[[NSNumber alloc]initWithInt:colorPatterIndex ]];
                         }];
